...
export class GenericDocumentResultFieldsPage extends ScanResultFieldsPage {
    ...
    override async ngOnInit() {
        this.genDocumentResult = JSON.parse(
    ...
    override loadResultFields(): ScanResultFieldsSection[] {
        if (this.genDocumentResult.fields) {
            ...
            Object.entries(this.genDocumentResult.fields).forEach(
                ([_key, _value]) => {
                    if (_key === 'mrz') {
                        if (_value) {
                            resultFields.push(
                                ...this.getMRZFields(
                                    _value as MrzDocumentResult,
                                ),
                            );
                        }
                    } else {
                        const field: ScanResultField = { key: _key };
                        ...
                        if (_key.endsWith('Uri')) {
                            if (_value) {
                                ...
                                if (_key === resultImageField.key) {
                                    resultImageField.fieldPhotoPreviewWebViewPath =
                                        Capacitor.convertFileSrc(_value);
                                }
                            }
                        } else if (_value) {
                            const genDocField = _value as GenericDocumentField;
                            field.value = `${genDocField.text} ${genDocField.confidence
                                ? '(confidence: ' +
                                genDocField.confidence.toFixed(2) +
                                ')'
                                : ''
                                }`;
                        }
            ...
            return [
                ...
                { title: 'Fields', fields: resultFields },
            ];
        } else {
    ...
    private getMRZFields(mrzValue: MrzDocumentResult): ScanResultField[] {
        const genDocMRZFields: ScanResultField[] = [];

        Object.entries(mrzValue).forEach(([_key, _value]) => {
            const field: ScanResultField = { key: `MRZ.${_key}` };
