import { DocumentScanningFlow, SBError, ScanbotDocument } from 'capacitor-plugin-scanbot-sdk';

async function handlingRtuuiErrors() {
  try {
    const result = await ScanbotDocument.startScanner(new DocumentScanningFlow());
