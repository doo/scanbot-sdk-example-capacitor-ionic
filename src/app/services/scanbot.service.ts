...
export const BARCODE_DOCUMENT_FORMATS_ENABLED_KEY =
...
export class ScanbotService {
    ...
    async initializeSDK() {
        ...
        try {
            const result = await ScanbotSDK.initializeSDK({
                ...
                fileEncryptionMode: this.FILE_ENCRYPTION_ENABLED
                    ? 'AES256'
                    : undefined,
                fileEncryptionPassword: this.FILE_ENCRYPTION_ENABLED
    ...
    async scanBarcode(): Promise<BarcodeResult> {
        return ScanbotSDK.startBarcodeScanner({
            acceptedDocumentFormats:
                (await this.isBarcodeDocumentFormatsEnabled())
                    ? await this.getAcceptedBarcodeDocumentFormats()
                    : [],
    ...
    async isLicenseValid(): Promise<boolean> {
        ...
        if (!licenseInfo.isLicenseValid) {
            this.utils.showWarningAlert(
                this.getErrorStringFromLicenseStatus(licenseInfo.licenseStatus),
            );
        }
    ...
    async detectBarcodesOnImage(
        imageUri?: string | undefined,
    ): Promise<DetectBarcodesOnImageResult> {
        let imageFileUri = imageUri;
    ...
    async detectBarcodesOnMultipleImages(
    ...
    async scanCheckFromImage(
        imageUri?: string | undefined,
    ): Promise<RecognizeCheckResult> {
        let imageFileUri = imageUri;
    ...
    async applyFilterOnImage(args: {
        filter?: ImageFilterType | undefined;
        imageUri?: string | undefined;
        showLoader?: boolean | undefined;
    }): Promise<ApplyImageFilterResult | undefined> {
        ...
        if (imageFilter) {
            ...
            return ScanbotSDK.applyImageFilter({
                filter: imageFilter,
                imageFileUri: imageFileUri,
            });
        } else {
    ...
    async applyFilterOnPage(args: {
        ...
        if (pageFilter) {
            ...
            return ScanbotSDK.applyImageFilterOnPage({
                page: args.page,
                filter: pageFilter,
            });
        } else {
    ...
    private async getAcceptedBarcodeDocumentFormats(): Promise<
        BarcodeDocumentFormat[]
        > {
    ...
    async isBarcodeDocumentFormatsEnabled(): Promise<boolean> {
        const prefValue = (
            await Preferences.get({ key: BARCODE_DOCUMENT_FORMATS_ENABLED_KEY })
        ).value;
