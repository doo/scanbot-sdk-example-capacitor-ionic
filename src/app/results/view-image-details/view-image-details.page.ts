...
export class ViewImageDetailsPage implements OnInit {
  ...
  async ngOnInit() {
    const pageId = this.activatedRoute.snapshot.paramMap.get('pageId') as string;
    const page = await this.preferencesUtils.getPageById(pageId);

    if (page) {
      this.updateResultUI(page);
    } else {
      this.utils.showErrorAlert('Result can\'t be found', () => { this.navController.back(); });
    }
  }
